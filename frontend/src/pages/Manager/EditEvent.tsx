...
const EditEvent: React.FC = () => {
    ...
    const fetchEventDetail = useCallback(async () => {
        ...
        try {
            ...
            if (result.success) {
                const d = result.data;
                // Format lại date để hiển thị trong input type="datetime-local"
                ...
                setFormData({
                    ...
                    capacity: {
                        maxVolunteers: d.capacity.maxVolunteers,
                        minVolunteers: d.capacity.minVolunteers
                    },
                    tags: d.tags || []
                });
            }
        ...
        <div style={{ display: 'flex', minHeight: '100vh', width: '100vw', backgroundColor: COLORS.BACKGROUND }}>
            <aside style={styles.sidebar}>
                ...
                <nav style={{ flex: 1, padding: '20px 12px' }}>
                    <div onClick={() => navigate("/manager/dashboard")} style={styles.navItem}><FaClipboardList style={{ marginRight: '12px' }} /> Pending Applications</div>
                ...
                <form onSubmit={handleSubmit} style={{ maxWidth: '900px', display: 'flex', flexDirection: 'column', gap: '24px', paddingBottom: '50px' }}>
                    ...
                    <div style={styles.card}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '15px' }}>
                            ...
                            <div><label style={styles.label}>Địa chỉ</label><input style={styles.input} value={formData.location.address} onChange={e => setFormData({...formData, location: {...formData.location, address: e.target.value}})} /></div>
                        </div>
                    </div>

                    {/* THỜI GIAN */}
                    <div style={styles.card}>
                        <h3 style={{ ...styles.cardHeader, color: COLORS.PRIMARY }}><FaCalendarAlt /> Thời gian & Hạn đăng ký</h3>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '15px' }}>
                            <div>
                                <label style={styles.label}>Ngày bắt đầu</label>
                                <input type="datetime-local" style={styles.input} value={formData.schedule.startDate} onChange={e => setFormData({...formData, schedule: {...formData.schedule, startDate: e.target.value}})} />
                            </div>
                            <div>
                                <label style={styles.label}>Ngày kết thúc</label>
                                <input type="datetime-local" style={styles.input} value={formData.schedule.endDate} onChange={e => setFormData({...formData, schedule: {...formData.schedule, endDate: e.target.value}})} />
                            </div>
                            <div>
                                <label style={styles.label}>Hạn đăng ký</label>
                                <input type="datetime-local" style={styles.input} value={formData.schedule.registrationDeadline} onChange={e => setFormData({...formData, schedule: {...formData.schedule, registrationDeadline: e.target.value}})} />
                            </div>
                        </div>
                    </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
                        ...
                        <div style={styles.card}>
                            <h3 style={{ ...styles.cardHeader, color: COLORS.PRIMARY }}><FaUserFriends /> Số lượng TNV</h3>
