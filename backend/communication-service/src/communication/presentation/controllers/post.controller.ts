...
export class PostController {
    ...
    async createPost(
        ...
        try {
            const imageUrls = images?.map((file) => `/uploads/posts/${file.filename}`) ?? [];

            const userProfile = await firstValueFrom(
                this.userClient.send('user.findByAuthId', { authId: userId })
            ).catch(err => {
                console.error('❌ Lỗi kết nối User Service:', err.message);
                return null;
            });

            if (!userProfile) {
                throw new BadRequestException('Không thể xác thực thông tin người dùng từ User-Service');
            }
            ...
            const post = await this.createPostUseCase.execute({
                eventId,
                content,
                authorId: userProfile.authId,
                authorName: userProfile.fullName || 'Người dùng',
            ...
            // Log lỗi chi tiết ra console của Docker để bạn nhìn thấy
            console.error('--- LỖI TẠO BÀI VIẾT ---');
            console.error(error.stack);

            throw new InternalServerErrorException({
                message: 'Lỗi xử lý server khi tạo bài viết',
